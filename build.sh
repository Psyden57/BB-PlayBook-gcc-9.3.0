	...
	mkdir -p ${QNX_TARGET}
 	...
 	if [ ! -f ${WORK_DIR}/have-patched-qnx ]; then
		...
		for file in crti.o crtn.o; do
			mv ${QNX_PREBUILT}/lib/${file} ${QNX_PREBUILT_GCCLIB}/../4.4.2 2>/dev/null || true
		done	
		for file in stdc++ asan gomp mudflap; do
			mv ${QNX_PREBUILT}/lib/lib${file}* ${QNX_PREBUILT_GCCLIB}/../4.4.2 2>/dev/null || true
		done
...
doPatch() {
	...
	if [ ! -f ${WORK_DIR}/have-patched-gcc ]; then
		...
		cd ${SRC_DIR}/${GCC_SRC}/libstdc++-v3 && autoreconf2.69 -f || exit 1
